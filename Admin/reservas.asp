...
sSql = "select COUNT(*) from reserva where existe = 1 and Not fila in ('30','31','32')"
Rs.Open sSql, Cn

sSql = " select asiento from RESERVA where estado = '1' and familia = '" & Usuario & "' "
Rs2.Open sSql, Cn
...
function ventanagrande(obj, color){
   asiento = obj.name;
   window.open("confirma.asp?asiento=" + asiento + "&color=" + color,"ventana1","width=420, height=180, top=150, left=580, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no")
}
...
<form name="frmAlumnos" method="post" action="entradas.asp">
               ...
               <td align="center">
                  <table width="100%" cellpadding="0" style="border-collapse: collapse" height="41">
                    <tr>
					   ...
					   <span class="auto-style27">** VIERNES</span><span class="auto-style26"> 22
         ...
         <table align=center border="0" cellpadding="1" cellspacing="0" bordercolor="#008AB8"  style="border-collapse: collapse" width="600">
            ...
            <tr>
              ...
              <td>&nbsp;&nbsp;</td>
               ...
               Else
                  sSql = "select columna, existe, asiento, estado, familia, final from reserva where fila = '" & Rs1("letra") & "'"
                  Rs.Open sSql, Cn
                  Do Until Rs.EOF
                     If Rs("estado") Then
                        reservas = reservas + 1
                     ...
                     Else
                        clase = "verde"
                     End If%>
                        <td nowrap><%If Rs("existe") Then%><input type="button" name="<%= Rs("asiento") %>" value="<%= Rs("asiento") %>" class="<%= clase %>" <%If clase = "rojo" Then%> disabled="disabled"<%End If%> <% If Rs("final") = "1" Then%> <% If clase = "azul" Then %> disabled="disabled" <% End If  %> <% End If %> onclick="ventanagrande(this, '<%= clase %>')" onmouseover="over(this)" onmouseout="out()"/></a><%End If%></td><%
                     Rs.MoveNext
                  Loop
                  Rs.Close
               End If%>
